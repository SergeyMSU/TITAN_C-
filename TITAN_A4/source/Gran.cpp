		...
		if (norm2(0.0, this->center[0][1], this->center[0][2]) < 10.0)
		{
			for (auto& i : this->yzels)
			{
				...
				for (auto& i : this->yzels)
				{
					...
					if (b1 == true || b2 == true || b3 == true)            // Сейчас вообще почти три ряда ячеек считаем Лаксом
					{
...
void Gran::Read_AMR(short int ni, short int nH, Phys_param* ph_param, bool need_refine, bool need_de_refine)
{
	...
	if (file_exists(ph_param->AMR_folder + "/" + name_f) && this->type == Type_Gran::Us)
	{
		...
		if (need_refine == true)
		{
			...
			if (nH == 8)
			{
				this->AMR[nH - 1][ni]->procent_signif = 15.0;  // 0.3
				this->AMR[nH - 1][ni]->procent_devide = 25.0; // 2.0
			}

			if (nH == 9)
			{
				this->AMR[nH - 1][ni]->procent_signif = 5.0;  // 0.3
				this->AMR[nH - 1][ni]->procent_devide = 11.0; // 2.0
			}
	...
	else
	{
		if (this->type == Type_Gran::Us)
		{
			...
			{
				this->AMR[nH - 1][ni]->AMR_resize(0.0, 35.0, -35.0, 35.0,
					-35.0, 35.0, 3, 6, 6);
			}
			else if (nH == 5 || nH == 6)
			{
