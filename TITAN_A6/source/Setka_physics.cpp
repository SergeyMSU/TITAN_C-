...
double Setka::Culc_Gran_Potok(Gran* gr, unsigned short int now, short int metod, string& name, const double& time)
{
	...
	if (this->phys_param->culc_plasma == true)
	{
		...
		if (true && (gr->type2 == Type_Gran_surf::HP))
		{
			...
			// === 3. Угол между тангенциальными полями ===
			...
			// фактор пересоединения
			double ftheta = 0.5 * (1.0 - costhe);

			// === 4. Эффективные параметры ===
			...
			// === 5. Скорость втекания ===
			double Mrec = 0.1;            // можно варьировать
			double vin = Mrec * vA * ftheta;


			// === 6. Поток массы ===
			// ключевая формула
			double eps = 1.0;             // из Nakamura (0.5–1)
			double phi = 0.1;             // доля поверхности
			...
			// === 7. Эффективная скорость для импульса ===
			...
			// полная скорость (с учётом пересоединения)
			...
			// любой вектор не параллельный нормали
			...
			//double fluxBx = -(ny * Ez - nz * Ey);  // n × E
			//double fluxBy = -(nz * Ex - nx * Ez);
			//double fluxBz = -(nx * Ey - ny * Ex);


			// Добавляем поток энергии - пойтингов
...
void Setka::Culc_usual_rotors_in_cell_2(void)
{
	cout << "Start: Culc_usual_rotors_in_cell_from_interpol" << endl;
		...
		for (auto gr : cell->grans)
		{
			...
			for (auto ed : gr->edges)
			{
				...
				if (true)
				{
					for (auto gr_ : ed->grans)
					{
						Eigen::Vector3d gr_center;
						gr_center << gr_->center[0][0], gr_->center[0][1], gr_->center[0][2];
						if (gr_->cells.size() == 1)
						{
							if (gr_->cells[0]->type != cell->type)
							{
						...
						double dfdf = (gr_center - ed_center).norm();
						v_all += 1.0 / dfdf;
						Vec_all += Vec / (dfdf);
					}
				}
...
void Setka::Culc_usual_rotors_in_cell_from_interpol(void)
{
	cout << "Start: Culc_usual_rotors_in_cell_from_interpol" << endl;
...
#pragma omp critical (first) 
		...
		for (auto gr : cell->grans)
		{
			...
			for (auto ed : gr->edges)
			{
				...
				for(short int ikk = 0; ikk < 1; ikk++)
				{
					double dxt = dxt_[ikk];
					double dw = dw_[ikk];


					fine_int = SS.Get_param(ed->B->coord[0][0] + ll[0] * dxt, 
						ed->B->coord[0][1] + ll[1] * dxt, ed->B->coord[0][2] + ll[2] * dxt, parameters);
					if (fine_int == false)
					{
						fine_int = SS.Get_param(0.999 * ed->B->coord[0][0] + ll[0] * dxt,
							0.999 * ed->B->coord[0][1] + ll[1] * dxt, 0.999 * ed->B->coord[0][2] + ll[2] * dxt, parameters);
					...
					Vec_all[0] += parameters["Bx"] * dw;
					Vec_all[1] += parameters["By"] * dw;
					Vec_all[2] += parameters["Bz"] * dw;
				}
