		...
		if (plasma_culc_or_atoms == true)
		{
			...
			if (plasma_cond == 1)
			{
				...
				if (gr->normal[now][0] < -0.95) // Для задней границы
				{
					...
					if (norm2(0.0, gr->center[now][1], gr->center[now][2]) < 50.0)
					{
						//par_left["Vx"] = -15.0; // this->phys_param->Velosity_inf;
						par_left["p"] /= 2.0; // this->phys_param->Velosity_inf;
					}
				}
				else
				{
		...
		if (plasma_culc_or_atoms == true && this->phys_param->TVD == true && A->is_TVD == true && B->is_TVD == true)
		{
			...
			if (MA > 5.0 && MAA > 5.0 && MB > 5.0 && MBB > 5.0)
			{
				...
				if (true)
				{
					...
					if (the1 > const_pi / 9 && the1 < 8 * const_pi / 9)  // the > 20  the < 160   Вне оси Z
					{
			...
			else if (bb_ && AA->type == Type_cell::Zone_1 && A->type == Type_cell::Zone_1 &&
			...
			else if (bb_ && AA->type == Type_cell::Zone_1 && A->type == Type_cell::Zone_2 &&
					B->type == Type_cell::Zone_2 && BB->type == Type_cell::Zone_2)
			{
				for (auto& nam : this->phys_param->plasma_pui_name)
				{
