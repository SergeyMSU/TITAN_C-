...
public:
    ...
    double sglag_HP_k_sphere = 0.0001;  //0.001    // Сферическое сглиживание в головной части * 100 - ОТКЛЮЧЕНО
    ...
    bool save_AMR = true;        // Нужно ли сохранять посчитанные функции распределения?
    bool culc_AMR = true;        // Нужно ли считать функции распределения?
    bool refine_AMR = true;      // Нужно ли мельчить посчитанные функции распределения?
    unsigned int N_per_gran = 150000;  // Сколько в среднем частиц вылетает с каждой грани
    bool culc_cell_moments = false;    // Нужно ли считать моменты в ячейках?
