...
public:
    ...
    bool null_bn_on_HP = false;   // Для ячеек рядом с HP обнуляем нормальную компоненту магнитного поля
    bool bn_in_p_on_HP = false;   // Для ячеек рядом с HP записываем магнитное поле в давление
